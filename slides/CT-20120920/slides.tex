...
\begin{document}
  ...
  \mysection{Motivation} % {{{
    ...
    \myframe[Fazit]{
        \begin{itemize}
            ...
            \item Oft keine Beachtung der realen Last des Nameservers
  ...
  \mysection{salbnet} % {{{
    ...
    \myframe[Konzept - Entwurf]{
        \textbf{Messwerte}
            \begin{tabular}{rl}
                ...
                $q_{current}$ &	aktuell belegte Speicherplatz in der UDP-Receive-Queue\\
    ...
    \myframe[Implementierung]{
        \textbf{Experiment}
        \begin{enumerate}
            \item Sende DNS-Nachricht mit steigender Größe
  ...
  \myframe{
      \begin{itemize}
          \item Vergleich von \textit{Weighted-Round-Robin (wrr)} und \textit{salbnet}
  ...
  \myframe[System]{
      \begin{description}
          \item[Client:] node015 -- Intel Xeon E5520 @ 2,27 GHz (Quad-Core)\\12 GB RAM
          \item[Lastverteiler:] ib1 -- 2 $\times$ AMD Opteron 244 @ 1,8 GHz (Single-Core)\\4 GB RAM
          \item[Backend:] 
              \begin{itemize}
                  \item ib4 -- 2 $\times$ Opteron 244 @ 1,8 GHz (Single-Core)\\4 GB RAM\\
                  \item ib6 -- Intel Pentium @ 2,8 GHz (Single-Core)\\4 GB RAM
                  \item ib8 -- Intel Xeon 3040 @ 1,86 GHz (Dual-Core)\\4 GB RAM
                  \item Receive-Queue: 25165824 Byte (24 MByte)
              \end{itemize}
      \end{description}
  }
    ...
    \myframe[Zusammenfassung] {
        \begin{itemize}
            \item Unterstützung von DNS-Nachrichten über UDP für \textit{salbnet} und
                \textit{servload}
            \item \textit{salbnet} unterstütz den Nameserver \textit{BIND}
            \item \textit{salbnet} Abschätzung der Größe von DNS-Paketen ist vermutlich auf Kernel 2.6.18
                beschränkt
            \item \textit{servload} unterstütz \textit{BIND}-Logs
        ...
        \begin{itemize}
            \item salbnet
                \begin{itemize}
                    ...
                    \item Portierung des \textit{salbnet}-Kernelmodul
