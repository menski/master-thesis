  ...
  % chapter Grundlagen }}}
	...
	einem Standard-Algorithmus vergleichen.
		...
		Folgenden werden die genauen Änderungen am Quellcode beschrieben.
			...
			\ref{lst:metric-max-udp} wird in Zeile 1 überprüft, ob bereits die maximale Kapazität der
			...
			\lstinputlisting[firstline=32,lastline=43,caption={Bestimmung der maximalen Kapzität der UDP-Receive-Queue (\texttt{linux/client\_metric.c} aus salbd)},label=lst:metric-max-udp]{listings/client_metric.c}

			% subsection Maximale Kapazität der UDP-Receive-Queue }}}

			\subsection*{Aktuell belegter Speicherplatz in der UDP-Receive-Queue} % {{{

				Da zur Ermittlung des aktuell belegten Speicherplatzes in der UDP-Receive-Queue, wie in
				...
				\ref{lst:metric-udp-proc} die Datei in Zeile 1 geöffnet und in Zeile 6 wird die Kopfzeile
				übersprungen. Danach wird in einer Schleife ab Zeile 11 der UDP-Socket des BIND Servers
				gesucht. Dazu wird in Zeile 13 die aktuelle Zeile der Datei eingelesen. Danach wird in Zeile
				18 die Adresse und der Port des Sockets, der eben gelesen Zeile, in einer
				\texttt{sockaddr\_storage}-Datenstruktur gespeichert. Diese wird anschließend in der Zeile
				24 mit der in salbd konfigurierten Adresse verglichen. Wurde der gesuchte Socket gefunden,
				wird der belegte Speicherplatz in Zeile 25 in der Variablen \texttt{current} der Metrik
				gespeichert und die Schleife in Zeile 27 verlassen.

			\lstinputlisting[lastline=29,caption={Bestimmung des belegten Speicherplatz in der UDP-Receive-Queue (\texttt{linux/client\_metric.c} aus salbd)},label=lst:metric-udp-proc]{listings/client_metric.c}
			...
			\subsection*{Median der Paketgrößen in der UDP-Receive-Queue} % {{{
				...
				weiter genutzt werden. Das neue Feld enthält dabei die Größe des DNS-Pakets in Byte. Die
				neue Funktion \texttt{recvmsg} ist in Listing \ref{lst:nethookc} aufgeführt. In Zeile 8 wird
				die Speicheradresse der originalen \texttt{recvmsg}-Funktion aus der \texttt{libc} ermittelt
				und gespeichert. Danach wird in Zeile 15 diese Funktion aufgerufen und der Rückgabewert
				gespeichert. Dieser Rückgabewert entspricht der Größe des DNS-Pakets, welcher anschließend
				in Zeile 21 in das neue Feld \texttt{recv\_size} der \texttt{nethook\_data}-Datenstruktur
				gespeichert wird. Zum Schluss wird der Rückgabewert der originalen
				\texttt{recvmsg}-Funktion zurückgegeben.
			...
			\lstinputlisting[caption={Auszüge der Funktion \texttt{recvmsg} aus \texttt{nethook.c} der libnethook},label=lst:nethookc]{listings/nethook.c}
			...
			\subsection*{Verworfene Anfragen seit der letzten Credit-Meldung} % {{{
				...
				systemweit verworfen worden. Danach wird in Zeile 14 die Schleife verlassen.
			...
			Nach dem alle Werte für die Receive-Queue-Metrik aus Abschnitt \ref{sec:entwurf} ermittelt
			wurden können nun die Hard- und Soft-Credits bestimmt werden. Dazu wird die in Abschnitt
			\ref{sec:anforderungen} entwickelte Formel \ref{eq:recvmsg} benötigt, mit der der
			...
			bereits 2 Messwerte für die Anzahl der verworfenen Pakete vorliegen. Ist dies der Fall wird in
			Zeile 3 die Differenz gebildet und somit die Anzahl der verworfenen Anfragen seit der letzten
			Credit-Meldung ermittelt. Ist diese größer 0 werden die Hard- und Soft-Credits in Zeile 4-5
			auf 0 gesetzt. Wurden keine Anfragen verworfen, berechnen sich die Hard-Credits in Zeile 8 aus
			der Differenz der maximalen Größe der Receive-Queue und des aktuell belegten Speicherplatzes
			geteilt durch den Median der Paketgrößen in der Receive-Queue (siehe Formel \ref{eq:credits}).
			Die Berechnung der Soft-Credits in Zeile 9 erfolgt ebenfalls nach der Formel \ref{eq:credits}
			nur das statt dem aktuell belegten Speicherplatz der Receive-Queue der Median der letzten 16
			Erhebungen verwendet wird.
			...
			\ref{lst:auslastung} an, wie oft das größte bisher verarbeitet DNS-Paket in der Receive-Queue
			gespeichert werden könnte, und \texttt{current}, wieviele Pakete aktuell in der Receive-Queue
			gespeichert sind, ausgehend vom Median der Paketgrößen der letzten 16 verarbeiteten DNS-Pakete.
			Mit Hilfe dieser Werte wird anschließend, ausgehend von der gewählten Meldestrategie,
			entschieden ob Credits an den Lastverteiler gemeldet werden müssen.
