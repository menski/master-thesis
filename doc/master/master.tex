...
\renewcommand{\maketitle}{
  ...
  \thispagestyle{empty}
	...
	Diese Masterarbeit befasst sich mit der Lastverteilung für DNS-Cluster. Im Mittelpunkt steht
...
\begin{document}
	...
	\label{cha:einleitung}
		...
		Domain-Name ist eine Zeichenkette, welche sich Menschen leicht einprägen können im Gegensatz
		...
		ebenfalls an viele andere Netzwerkdienste gestellt werden. Um diese Anforderungen zu erfüllen,
		...
		Diese Arbeit beschäftigt sich mit dem Thema der Lastverteilung für DNS-Cluster. Dazu wird ein
		...
		Dieses System ist in dem Sinne selbst-adaptiv, dass der Lastverteiler keine eigenständigen
		...
		beschrieben.  Es wird hierbei auf Besonderheiten und Probleme bei der Implementierung
	...
	\label{cha:grundlagen}
		...
		Softwaresysteme eingegangen, welche aktuell zur Bereitstellung eines DNS-Servers genutzt werden.
			...
			Netzwerkteilnehmer über den zugewiesenen Namen zu kontaktieren. Der Nutzer bräuchte im
			...
			wurde. Dieses Verzeichnis wurde zentral zur Verfügung gestellt, sodass jeder Netzwerknutzer
			...
			Punkte getrennt sind und jeweils einem Knoten im Baum entsprechen. So besteht die Domain
			\texttt{cs.uni-potsdam.de} zum Beispiel aus den Subdomains \texttt{cs}, \texttt{uni-potsdam},
			...
			\subsubsection*{Resolver} % {{{
				...
				In Abbildung \ref{fig:bsp-request} ist der Ablauf der Beispielanfrage dargestellt. Der
			...
			\label{sub:software}
				...
				lokalen und Firmennetzwerken vor. Ein besonderes Merkmal des Microsoft DNS Servers ist,
		...
		Besonderheiten und existierende Verfahren der Lastverteilung von DNS-Verkehr erklärt werden.
			...
			Unabhängig von der gewählten Softwarelösung ist der gewählte Algorithmus zur Lastverteilung
			...
			nicht immer der Fall sein und außerdem ist es möglich, dass der Client über einen DNS-Server
			Anfragen stellt, welcher selbst nicht geografisch nah ist. Dadurch kann es zu negativen
			Effekten kommen.

			Auch Kombinationen der Server- und DNS-Lastverteilung sind möglich. Dabei wird ein
			...
			die Abarbeitung einer Anfrage benötigt. Dadurch kann die DNS-Antwort entsprechend der
			...
			ist das genutzte Transportprotokoll UDP, statt TCP, und die Nachrichten dürfen nicht größer als
  ...
  % chapter Grundlagen }}}
	...
	anscheinend oft für Denial of Service Angriffe genutzt. Dabei ist nicht der Root-Server das
	...
	Eine zusätzlich Art der Verteilung von DNS-Anfragen sind Anycast-Adressen. Wie in Kapitel
	...
	UltraDNS sogar bei \unit[45]{\%} bis \unit[53]{\%} der Anfragen. Allerdings zeigte sich, dass die
	...
	Clients stammte.  Allerdings gab es auch das Beispiel des F-Root-Server in Los Angeles, USA,
	...
	\label{cha:salbnet}
		...
		\label{sec:meldestrategien}
			...
			Der Dynamic Pressure Relieve Algorithmus kombiniert alle vorherigen Algorithmen. Es existieren
		...
		\label{sec:Benchmark}
			...
			Um \textit{salbnet} auf Clustern zu testen, wurde der Benchmark servprep/servload
		...
		\label{sec:salbnet-fazit}
			...
			Zusätzlich zu \textit{salbnet} wurde der Benchmark servload entwickelt, welcher in
			...
			servload-Benchmark generiert werden, ein wichtiger Grund für die Wahl.
	...
	\ref{cha:implementierung} beschrieben.
		...
		\ref{sec:meldestrategien}) werden die Credits in dynamisch berechneten Intervallen gemeldet. Das
		...
		In diesem Kapitel sollte eine Metrik zur Bestimmung der Lastsitutation eines DNS-Servers
		...
		Kapitel wird nun die Implementierung der Metrik beschrieben.
	...
	einem Standardalgorithmus vergleichen.
		...
		und die Ausgaben aller Kindprozesse in einzelnen Dateien gespeichert. In der Ausgabedatei des
		...
		DNS-Paket \unit[45]{Byte} groß. Somit ist es möglich, den Funktionsaufruf des BIND abzufangen
		...
		\lstinputlisting[float,caption={/proc/net/snmp Protokollinformation für SNMP-Programme},label=lst:proc-snmp]{listings/proc-snmp.txt}
		...
		Dieser Abschnitt stellt die konkrete Implementierung, der in Abschnitt \ref{sec:entwurf}
	...
	\label{cha:messungen}
		...
		\label{sec:experiment}
			...
			\begin{itemize}
				\item Systemparameter (statisch)
					\begin{itemize}
						...
						\item Netzwerkstack des Betriebssystems
			...
			(\textit{wrr}) des LVS verwendet und \textit{salbnet} mit der Dynamic Pressure Relieve Meldestrategie. Als Gewichte
		...
		erfolgreich umgesetzt wurde. Die Implementierung funktioniert und hat in den Tests eine hohe
	...
	Ausblick auf mögliche Verbesserungen und Weiterentwicklungen gegeben.
		...
		Nach der erfolgreichen Implementierung wurden mehrere Messungen durchgeführt. Sie hatten zum
