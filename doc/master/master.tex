...
\renewcommand{\maketitle}{
  ...
  \thispagestyle{empty}
	...
	Konzepts überprüft. Auf Basis dieser Ergebnisse wird für salbnet eine UDP-Erweiterung
...
\newcommand{\makeauthorship}{
  ...
  \thispagestyle{empty}
  Hiermit erkläre ich, dass ich die vorliegende Arbeit selbständig angefertigt, nicht anderweitig zu Prüfungszwecken vorgelegt und keine anderen als die angegebenen Hilfsmittel verwendet habe. Sämtliche wissentlich
...
\begin{document}
	...
	\label{cha:einleitung}
		...
		Domain-Name ist eine Zeichenkette, welche Menschen sich leichter merken können als eine
		...
		Anschließend werden in Kapitel \ref{cha:arbeiten} einige Arbeiten aufgeführt, welche sich mit
		DNS-Clustern und Lastsituationen in eben diesen befassen. Das zu erweiternde System zur
		selbst-adaptiven Lastverteilung salbnet \cite{zinke2012,salbnet} wird in Kapitel
  ...
  % chapter Grundlagen }}}
	...
	In \cite{brclne2001} wurde Anfang 2001 die Anfragen an ein DNS-Cluster des Root-Servers F
	gemessen. Der Cluster bestand aus zwei DEC Alpha Systemen und eine Cisco-Router, welcher CEF
	(Cisco Express Forwarding) zur Lastverteilung nutzte. Die Messungen zeigten, dass es zu
	Lastspitzen mit bis zu 5000 Anfragen pro Sekunde kam und der Root-Server ca. \unit[93]{\%} der Anfragen
	sofort beantworten konnte. Die restlichen \unit[7]{\%} waren Anfragen konnte nicht beantwortet werden, weil
	zum Beispiel die DNS-Anfragen falsch gestellt wurden. Sehr interessant sind die Ergebnisse
	bezüglich Nameserver bzw. Clients, welche nicht korrekt auf Antworten oder Fehler reagieren. So
	wurde ein Client im Log entdeckt, welcher 2.112.962 in einer Stunde die selbe Anfrage stellte.
	Insgesamt konnte festgestellt werden das in einem Log von 2 Stunden Dauer 29 Millionen Anfragen
	eingingen und davon \unit[85]{\%} wiederholte Anfragen waren (selber Client und selbe Anfrage). Außerdem
	wurden in einen ein Stunden Log 143.783 verschiedene ungültige TLDs angefragt. Der Grund für viele
	falsche Anfragen oder wiederholte Anfragen, scheint fehlerhafte Implementationen des DNS-Standards
	\cite{rfc1034, rfc1035} zu sein oder falsch konfigurierte Softwaresysteme. Diese fehlerhaften
	DNS-Anfragen machen insgesamt ungefähr einen Anteil von \unit[14]{\%} aus. Außerdem werden Root-Server
	anscheint oft oft Denail of Service Angriffe genutzt. Dabei ist nicht der Root-Server das Ziel,
	sondern er wird als Reflektor genutzt. Dazu werden eine große Menge an Anfragen mit veränderter
	Absendeadresse geschickt. Diese werden dann alle vom Root-Server beantwortet und an das
	Angriffsziel geschickt. Dieses wird dann mit DNS-Antworten beschossen, welche er nie angefordert
	hat.

	In \cite{isetal2005} werden die DNS-Anfragen eines Virus in Japan gemessen. Dabei wurden die
	Anfragen des Virus japanischen ISP Open Computer Networks (OCN) analysiert. Der Virus greift einen
	Webserver an und erfragt dazu über das DNS die IP-Adresse des Ziels. Dies führte zu ca. 500
	Anfragen pro Sekunde nach dieser Adresse. Eine Entscheidung des Administrators des Angriffsziels
	zeigt die starken Abhängigkeiten innerhalb des DNS. Die Veränderung eines Eintrags auf einem
	Nameserver kann auch andere Nameserver im System betreffen. So entfernte der Zuständige des
	Angriffsziels den entsprechenden Eintrag des Webservers aus dem DNS. Dies führte dazu das die
	Anfragen des Virus nach der IP-Adresse nicht mehr beantwortet werden konnte und dieser
	anschließend immer wieder DNS-Anfragen sendete. Dabei wurde die Gültigkeitsdauer der Antwort, dass
	der Server nicht exisitert, von vielen Clients ignoriert, ob dies vom Virus ausging oder die
	Nutzer dies selber aus anderen Gründen an ihren Systemen deaktivierten ist unklar. Jedenfalls
	stiegen die DNS-Anfragen beim Nameserver des ISPs auf bis zu über 4500 pro Sekunde an.
	Ausschließlich nach der Adresse des Zielsystems. Erst durch das wiedereinfügen des DNS-Eintrags
	wurde die Last auf dem ISP-Nameserver geringer. Der Angriff wurden schlussendlich dadurch gelöst,
	dass statt der Adresse des Webservers eine Schein-IP-Adresse zurückgeliefert wurde. Alle Anfragen
	an diese IP-Adresse wurden dann vom Router verworfen.

	% TODO: DNS Cluster

	In \cite{sapate2005} wird das Verhalten von Anycast-DNS-Servern Ende 2004 untersucht. Dazu werden die
	...
	alle ihre Server. Die Messungen zeigen, dass bei den Root-Server nur \unit[15]{\%}-\unit[27]{\%} der Anfragen an den
	geografisch nähsten Cluster geschickt gesendet werden. Im Vergleich dazu werden \unit[45]{\%} bis \unit[53]{\%} der
	Anfragen an UltraDNS, an den geografisch nähsten Server gesendet. Allerdings, zeigte sich die
	DNS-Cluster der Root-Server eine höhere Erreichbarkeit hatten. Zusammenfassend ist zu sagen, dass
	Anycast sinnvoll ist um DNS-Anfragen zu verteilen, allerdings Anycast stark von dem Netzwerk und
	den BGB-Routen abhängt. Somit können Netzwerkausfälle die Routen von Anycast-Anfragen stark
	beeinflussen.

	In \cite{lietal2007} wurden Daten von Anfang 2006 der Anycast Root-Server C, F und K untersucht.
	Hierbei wurde ein Zeitraum von 47,2 Stunden, annähernd 2 Tage, betrachtet. Es konnte gezeigt
	werden, das der größte Teil der DNS-Server Anfragen von geografisch nahen Clients erhielt.
	Allerdings gab es auch wie einen F-Root-Server in Los Angeles, USA, welcher \unit[88]{\%} seiner
	Anfragen aus Asien erhielt. Oder ein K-Root-Server nähe Delhi, Indien, welcher über
	\unit[75]{\%} seiner Anfragen aus Nordamerika, Südamerika und Europa erhielt. Hingegen waren die
	Anfragen vom K-Root-Server in Tokyo, Japan, zu \unit[99,7]{\%} aus Japan und \unit[75]{\%}
	waren von Clients mit einer Entfernung unter \unit[1000]{km}. Zusätzlich konnte gezeigt
	werden, dass in dem betrachtet Zeitraum, die meisten Clients immer den selben DNS-Server
	befragen, so wechselten z.B. beim C-Root-Server nur \unit[1,7]{\%} der Clients den
	Nameserver. Bei dem F und K-Root-Server waren es \unit[1,4]{\%} bzw. \unit[4,7]{\%}.
