  ...
  % chapter Grundlagen }}}
	...
	einem Standard-Algorithmus vergleichen.
		...
		Folgenden werden die genauen Änderungen am Quellcode beschrieben.
			...
			direkt in die \texttt{maximum}-Variable der Metrik gespeichert. Wenn keine Fehler aufgetreten
			...
			\subsection*{Aktuell belegter Speicherplatz in der UDP-Receive-Queue} % {{{
				...
				\ref{lst:metric-udp-proc} die Datei in Zeile 2 geöffnet und in Zeile 7 wird die Kopfzeile
				übersprungen. Danach wird in einer Schleife ab Zeile 12 der UDP-Socket des BIND Servers
				gesucht. Dazu wird in Zeile 14 die aktuelle Zeile der Datei eingelesen. Danach wird in Zeile
				19 die Adresse und der Port des Sockets, der eben gelesen Zeile, in einer
				\texttt{sockaddr\_storage}-Datenstruktur gespeichert. Diese wird anschließend in der Zeile
				25 mit der in salbd konfigurierten Adresse verglichen. Wurde der gesuchte Socket gefunden,
				wird der belegte Speicherplatz in Zeile 26 in der Variablen \texttt{current} der Metrik
				gespeicher und die Schleife in Zeile 27 verlassen.
			...
			\subsection*{Median der Paketgrößen in der UDP-Receive-Queue} % {{{
				...
				gespeichert wird. Am Ende der Funktion wird der Rückgabewert der originalen
			...
			\subsection*{Verworfene Anfragen seit der letzten Credit-Meldung} % {{{
				...
				aktuelle Anzahl der verworfenen Anfragen in Variablen \texttt{current} der Metrik
			...
			\lstinputlisting[firstline=3,lastline=19,caption={Bestimmung der Hard- und Soft-Credits (\texttt{client\_report.c} aus salbd)},label=lst:metric-udp-proc]{listings/client_report.c}
			
			% subsection Bestimmung der Hard- und Soft-Credits }}}

			\subsection*{Bestimmung der Auslastung der Receive-Queue} % {{{

			\lstinputlisting[firstline=22,caption={Bestimmung der maximalen und aktuellen Auslastung der UDP-Receive-Queue	(\texttt{client\_report.c} aus salbd)},label=lst:metric-drops-udp]{listings/client_report.c}
