...
\begin{document}
	...
	\label{cha:grundlagen}
		...
		\label{sec:grundlagen-fazit}
			...
			In diesem Kaptiel wurde die Enstehung und Bedeutung des \dns{} erklärt. Es zeigt sich, dass
			ohne DNS die heutige Art der Nutzung des Internets nicht möglich wäre. Durch DNS wird eine
			schnelle und einfache Ermittlung von IP-Adresse realisiert. Daher muss das System jederzeit
			verfügbar sein und geringe Latenzen ermöglichen. Dafür werden die Informationen im DNS in
			einer verteilten Baumstruktur gespeichert. Diese ermöglicht das einfache delegieren von
			Verantwortungen. Allerdings führt diese spezielle Struktur dazu, dass auf dem Wurzelknoten
			(Root-Domain) und seinen Kindknoten (TLDs) die größte Last liegt. Diese Last nimmt mit
			steigenden Tiefe immer stärker ab. Folglich sind für die verschiedenen Ebenen des DNS
			jeweils andere Verfahren zur Lastverteilung geeignet. Es wurden die Konzepte der
			...
			salbnet eine Server-Lastverteilung realisiert. Somit sind private Netzwerk und kleine
			Firmennetzwerke kein Anwendungsbereich für salbnet. Bei Root-Server, TLDs und großen SLDs ist
			es realistischer, dass DNS-Cluster genutzt werden, um die hohe Anzahl an DNS-Anfragen
			abzuarbeiten. Dementsprechend ordnet sich salbnet bei diesen großen DNS-System ein, bei denen
  ...
  % chapter Grundlagen }}}
	...
	% TODO: On the Use of Anycast in DNS
	- Verhalten von Anycast im DNS
	- 15% von 55% beantwortet vom topologisch nächsten
	- 10% Verzögerung von 100ms
	...
	\label{cha:messungen}
		...
		erfolgreich umgesetzt wurde. Die Implementation funktioniert und hat in den Tests ihre
		Leistungsfähigkeit unter Beweis gestellt. In allen verglichenen Metriken ist salbnet dem
		wrr-Algorithmus überlegen. Nur die CPU-Auslastung der ib6 war im Vergleich zu den
		anderen Backend-Servern zu hoch. Dies kann an zu optimistischen oder zu späten Credit-Meldungen
